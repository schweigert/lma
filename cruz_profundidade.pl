...
inicio :-  
    pt_origem(X),
    busca([X], Solucao), 
   /* X  eh o no inicial, logo entra na lista */
    reverse(Solucao, Sol_invertida), 
    write_L(Sol_invertida),
    length(Sol_invertida, Total),
    pt_destino(Y), nl,
    write('Onde a posicao inicial era: '), write(X), nl,
    write('E a posicao final era: '), write(Y), nl,
    write('Total de estados: '), write(Total), 
    Aux is (Total -1), nl,
    write('Total de movimentos: '), write(Aux), !.

inicio:- nl, write('Sem solucaoo ... caminho nao encontrado!').
...
%% cond. de parada.... o noh corrente eh o desejado
